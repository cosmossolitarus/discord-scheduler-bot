...
Builds the structured state the LLM needs to make decisions. Contains:
  - the user's submission status (screenshot, availability summary, Speedups,
    premium resources, player ID)
  - their current assignments (post-publish only)
  - valid swap partners from @mentions (post-publish only)
...
async def build_state_envelope(
    ...
    # Post-publish additions: assignments + valid swap partners
...
def render_state_for_prompt(state: dict) -> str:
    ...
    if sub["speedups"]:
        ...
        parts.append(
            ...
            f"general={r['general_days']:.2f} (general split by {r['generic_split']})"
        )

    if sub["resources"]:
        r = sub["resources"]
        parts.append(
            f"  Premium resources: TTG={r['ttg']:.0f}, TG={r['tg']:.0f}, Dust={r['dust']:.0f}"
        )
    ...
    if state["phase"] == "published":
        ...
        if not state["assignments"]:
            ...
            for a in state["assignments"]:
                ...
                boundary = (
                    " [BOUNDARY SLOT: first 15 min uses Day 1 Speedups, "
                    "last 15 min uses Day 2 Speedups]"
                    if a["is_boundary"] else ""
                )
                parts.append(
        ...
        if state["valid_swap_partners"]:
            ...
            parts.append("")
            parts.append(
                "VALID SWAP PARTNERS IN THIS MESSAGE: "
                "(none — user did not @mention an assigned player)"
            )
